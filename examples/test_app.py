...
from model_fitting.model_fitting_app import Model_Fitting_App, launch
...
class TestHost(tk.Tk):
    ...
    def __init__(self):
        ...
        self._counter_label: ttk.Label = None
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        # ---- 主题切换 ----
        theme_f = ttk.Frame(f)
        theme_f.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(theme_f, text="主题：", font=("Microsoft YaHei", 9)).pack(side=tk.LEFT, padx=(0, 5))
        self._style = ttk.Style(self)
        available_themes = self._style.theme_names()
        self._theme_var = tk.StringVar(value=self._style.theme_use())
        theme_cb = ttk.Combobox(theme_f, textvariable=self._theme_var,
        ...
        self._counter_label = ttk.Label(f, text="打开的子窗口数: 0",
    ...
    def _update_counter(self):
        if self._counter_label is not None:
            self._counter_label.config(text=f"打开的子窗口数: {len(self._child_windows)}")
